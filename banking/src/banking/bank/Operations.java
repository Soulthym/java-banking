...
public class Operations {
	...
	public Account createNewAccount (String name, int age) throws InputException {
		...
		_fileLogger ("PROGRAM", "The program is now used by " + userAccount.getUserName());
		return userAccount;
	}
	...
	public void depositMoney (Account account) throws InputException {
		...
		_fileLogger ("PROGRAM", "Account : " + account.getUserName() + " , age : "+ account.getUserAge() + " has been credited of" + account.getTotal() + "€.");
	}
	...
	public void manageLoans (Account account) throws RuleException, InputException {
		...
		switch (userChoice) {
			...
			case 2 : {
				...
				_fileLogger ("PROGRAM", "Account : " + account.getUserName() + " , age : "+ account.getUserAge() + " has a new loan of " + amount + "€, with a rate of " + taux + "and a time of : " + time);
			}
		}
	}
	
	//throws InputException & RuleException ?
	public void transferMoney (Account account) throws RuleException, InputException {
		...
		for(int i = 0; i < _accounts.size(); i++) {
			if (nameToTransfer.equals(_accounts.get(i).getUserName()) && ageToTransfer==_accounts.get(i).getUserAge()) {
				...
				_fileLogger ("PROGRAM", moneyToTransfer + "€ has been transfered from " + account.getUserName() + ", " + account.getUserAge() + " to " + accountToTransfer.getUserName() + ", " + accountToTransfer.getUserAge());
			}
		}
	}
